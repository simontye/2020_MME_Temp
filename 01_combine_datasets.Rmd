...
fish <- mn.fish %>%
  ...
  mutate_at(c(13:76, 78:86), ~replace(., is.na(.), 0)) # replaces NAs with 0s for all species/family observations, temperature categories, and fishkill causes
...
# The single argument to this function, pointsDF, is a dataframe in which:
...
df.historical[113:114] <- do.call('rbind', strsplit(as.character(df.historical$state), ','))
...
df.historical <- df.historical %>%
  ...
  mutate(state = ifelse(state == "minnesota", "mn", state),
    ...
    mutate_at(c(28:100), ~replace(., is.na(.), 0)) # replaces NAs with 0s for all species/family observations, temperature categories, and fishkill causes
...
df.historical <- df.historical %>%
  ...
  mutate(long.x = NULL, # removes old coordinates
         lat.x  = NULL) %>%
  rename(long = long.y, # keeps new coordinates
         lat  = lat.y)

## Add 2010 census data (Till method)
