...
#  mutate(long_round = round(long, 1),
#         lat_round =  round(lat, 1)) %>%
#  left_join(census.df, by = c("long_round", "lat_round")) %>%
#  dplyr::select(-long_round, -lat_round)

# Remove "county" from county names in census data
census$county <- str_sub(census$county, end = -8)

# Reformat census data
census <- census %>%
  rename_all(tolower) %>%
  mutate_all(tolower)

# Merge census data with historical df
df.historical <- df.historical %>%
  left_join(census, by = c("county", "state")) %>%
  mutate(pop_2010 = as.character(pop_2010))

# Remove commas from population estimates
df.historical$pop_2010 <- as.numeric(gsub(",","",df.historical$pop_2010))

# Save historical dataframe
write.csv(df.historical, "data/processed/df_historical.csv", row.names = FALSE)

# Remove unnecessary dataframes
rm(spatial, fish, df.historical)
```
...
## Remove missing air temperature data
df.future <- df.future %>%
...
# Remove commas from population estimates
df.future$pop_2010 <- as.numeric(gsub(",","",df.future$pop_2010))

# Save processed data
write_csv(df.future, "data/processed/df_future.csv")

# Remove unneccessary dataframes
rm(df.future, site.id, census, latlong2county)
```
