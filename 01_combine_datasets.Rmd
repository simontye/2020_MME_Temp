...
df.historical <- localities %>%
  ...
  dplyr::select(., -c("lat.x", "long.x", "id")) %>%
  rename(lat    = lat.y,
         long   = long.y) %>%
...
# Subset coordinates to find county, state of each waterbody
...
# Find county and state for each set of coordinates
df.historical$state <- as.vector(latlong2county(coordinates))

# Separate county and state columns
df.historical[66:67] <- do.call('rbind', strsplit(as.character(df.historical$state), ','))
...
# Remove "county" from county names in 2010 US census data
census$county <- str_sub(census$county, end = -8)

# Add census data
df.historical <- census %>%
  rename_all(tolower) %>%
  mutate_all(tolower) %>%
  left_join(df.historical, ., by = c("county", "state")) %>%
  mutate(pop_2010 = as.character(pop_2010)) %>%
  mutate(pop_2010 = as.numeric(gsub(",","", pop_2010)),
         county = NULL,
         state = NULL)
...
ecoregions.sp  <- SpatialPoints(ecoregions.sp)
...
df.historical <- df.historical %>%
  ...
  subset(., !is.na(state)) %>%
  mutate(gnis_nm = NULL,
         l3_code = substr(test$l4_code, 1, 2)) %>%
  mutate_at(c(26:53), ~replace(., is.na(.), 0))
...
rm(fish, df.historical,
   ...
   coordinates, localities, spatial)
```
...
```{r: Add future water temperature}
...
# Find county and state for each set of coordinates
df.future$state <- as.vector(latlong2county(coordinates))
...
df.future <- census %>%
  rename_all(tolower) %>%
  mutate_all(tolower) %>%
  left_join(df.future, ., by = c("county", "state")) %>%
  mutate(pop_2010 = as.character(pop_2010)) %>%
  mutate(pop_2010 = as.numeric(gsub(",","", pop_2010)),
         county = NULL,
         state = NULL)
...
rm(ecoregions, ecoregions.sp,
   ecoregions.mn, ecoregions.wi,
   census, coordinates, site.id,
   spatial.df, df.future)
```
