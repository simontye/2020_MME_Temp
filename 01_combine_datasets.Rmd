...
mn.fish <- mn.fish %>%
  ...
  mutate(
    ...
    dplyr::select(., c("event", "month", "year", "waterbody.name",
                     ...
                     "anthropogenic", "infectious", "cause.group"))
...
# Calculate number of columnaris events in MN
#test <- mn.fish.1 %>%
#  merge(., mn.fish.2, all = TRUE) %>%
#  filter(cause.group == "columnaris" | cause.group == "columnaris disease and aermonas septicemia)" | cause.group == #"columnaris with parasitic infection" | cause.group == "possible columnaris" | cause.group == "environmental stress with #columnaris")
...
wi.fish <- wi.fish %>%
  ...
  mutate_all(tolower) %>%
  dplyr::filter(date > "2003-01-01", date < "2013-12-31") %>%  # changed to match timespan of Phelps, removed last 6 months
  dplyr::rename(#month        = investigation.start.month,
  ...
  mutate_at(c(4, 5, 40), as.character) # convert event, month, and year to character
...
wi.fish[wi.fish$event == 64,   c(6:17, 19:37, 46:59)][is.na(wi.fish[wi.fish$event == 64,   c(6:17, 19:37, 46:59)])] = 0
wi.fish[wi.fish$event == 132,  c(6:17, 19:37, 46:59)][is.na(wi.fish[wi.fish$event == 132,  c(6:17, 19:37, 46:59)])] = 0
wi.fish[wi.fish$event == 3321, c(6:17, 19:37, 46:59)][is.na(wi.fish[wi.fish$event == 3321, c(6:17, 19:37, 46:59)])] = 0
wi.fish[wi.fish$event == 3284, c(6:17, 19:37, 46:59)][is.na(wi.fish[wi.fish$event == 3284, c(6:17, 19:37, 46:59)])] = 0
wi.fish[wi.fish$event == 2262, c(6:17, 19:37, 46:59)][is.na(wi.fish[wi.fish$event == 2262, c(6:17, 19:37, 46:59)])] = 0
wi.fish[wi.fish$event == 2259, c(6:17, 19:37, 46:59)][is.na(wi.fish[wi.fish$event == 2259, c(6:17, 19:37, 46:59)])] = 0
wi.fish[wi.fish$event == 2160, c(6:17, 19:37, 46:59)][is.na(wi.fish[wi.fish$event == 2160, c(6:17, 19:37, 46:59)])] = 0
wi.fish[wi.fish$event == 2033, c(6:17, 19:37, 46:59)][is.na(wi.fish[wi.fish$event == 2033, c(6:17, 19:37, 46:59)])] = 0
wi.fish[wi.fish$event == 1722, c(6:17, 19:37, 46:59)][is.na(wi.fish[wi.fish$event == 1722, c(6:17, 19:37, 46:59)])] = 0
wi.fish[wi.fish$event == 1508, c(6:17, 19:37, 46:59)][is.na(wi.fish[wi.fish$event == 1508, c(6:17, 19:37, 46:59)])] = 0
wi.fish[wi.fish$event == 1455, c(6:17, 19:37, 46:59)][is.na(wi.fish[wi.fish$event == 1455, c(6:17, 19:37, 46:59)])] = 0
wi.fish[wi.fish$event == 1399, c(6:17, 19:37, 46:59)][is.na(wi.fish[wi.fish$event == 1399, c(6:17, 19:37, 46:59)])] = 0
wi.fish[wi.fish$event == 1269, c(6:17, 19:37, 46:59)][is.na(wi.fish[wi.fish$event == 1269, c(6:17, 19:37, 46:59)])] = 0
wi.fish[wi.fish$event == 1136, c(6:17, 19:37, 46:59)][is.na(wi.fish[wi.fish$event == 1136, c(6:17, 19:37, 46:59)])] = 0
wi.fish[wi.fish$event == 107,  c(6:17, 19:37, 46:59)][is.na(wi.fish[wi.fish$event == 107,  c(6:17, 19:37, 46:59)])] = 0
...
wi.fish <- wi.fish.1 %>%
  ...
  mutate(month = as.numeric(month),
         fish_families = rowSums(.[, c(47:60)]), # sum families
         fish_species  = rowSums(.[, c(11:38)]), # sum species (exceptions below)
  ...
  mutate_all(na_if,"") %>% # replaces empty cells with NAs
  mutate_at(., vars(c(47:60)), list(~ ifelse(fish_families == 0, NA, .))) %>% # ensures that, if no fish families were observed, NAs are in all the species columns to reduce the number of false negatives
  mutate_at(., vars(c(11:38)), list(~ ifelse(fish_species == 0, NA, .))) %>% # ensures that, if no fish species were observed, NAs are in all the families columns to reduce the number of false negatives
  mutate(fish_families = ifelse(fish_families == 0, NA, fish_families),
...
## Number of columnaris events for WI
#test <- wi.fish %>%
#  mutate_all(tolower) %>%
#  rename_all(tolower) %>%
#  filter(detail.cause.type == "columnaris")
```
...
fish <- mn.fish %>%
  ...
  dplyr::select(., -c(10, 21:31, 42:77)) %>% # removes fish species
...
### Step 7: Add geographic, census, and ecoregion to historical dataset
...
# Replace NAs with 0s for families
df.historical <- df.historical %>%
...
### Step 10: Add geographic, census, and ecoregion to future dataset
