...
library(plyr)
library(remotes)
library(lakeattributes) # requires remote install above
library(data.table)
library(tidyverse)
library(sp)
library(rgeos)
...
# Load files
mn.fish      <- read.csv(file = "data/raw/fishkills/fishkills_mn.csv", head = TRUE, sep = ",")
wi.fish      <- read.csv(file = "data/raw/fishkills/fishkills_wi.csv", head = TRUE, sep = ",")
wi.site      <- read.csv(file = "data/raw/sites/WI_Site.csv", head = TRUE, sep = ",")
site.id      <- read.csv(file = "data/raw/sites/Site_ID.csv", head = TRUE, sep = ",")
hist.water   <- read.csv(file = "data/raw/temps/temp_water.csv", head = TRUE, sep = ",")
hist.air.1   <- read.csv(file = "data/raw/temps/prism_1.csv", head = TRUE, sep = ",")
hist.air.2   <- read.csv(file = "data/raw/temps/prism_2.csv", head = TRUE, sep = ",")
census       <- read.csv(file = "data/raw/census/2010_census.csv", head = TRUE, sep = ",")

# Function to get US county and state
...
# Save species and families
taxa <- c("centrarchidae", "percidae", "esocidae", "ictaluridae", "cyprinidae",
          "salmonidae", "osmeridae", "catostomidae", "sciaenidae", "lepisosteidae",
          "bullhead", "brown.trout", "largemouth.bass", "smallmouth.bass", "minnow",
          "cisco", "channel.catfish", "freshwater.drum", "northern.pike", "rainbow.smelt",
          "walleye", "carp", "crappie", "trout", "sunfish", "white.sucker",
          "yellow.perch", "muskies", "bass", "stonecat", "gizzard.gizzard.shad",
          "rockbass", "sauger", "gar", "whitefish")
...
# 3) Manually check events in May / October
mn.fish <- mn.fish %>%
  rename_all(tolower) %>%
  mutate_all(tolower) %>%
  ...
  mutate_at(c(taxa), as.numeric) %>%
  dplyr::rename(event   = event..,
         waterbody.name = waterbody,
         lat            = y,
         long           = x,
         cause.detail   = final.diagnosis) %>%
  mutate(dummy = 1,
         cause.groups = cause.detail) %>%
  spread(as.numeric(cause.groups), dummy, fill = 0) %>%
  dplyr::rename(unknown = V1) %>% # unknown diagnoses (blank cells) were moved to V1
  mutate(
    # Anthropogenic
    anthropogenic = ifelse(`biofueling bleach` >= 1 | `chemica ` >= 1 | `chemical smell suspected spill` >= 1 | `discharge power plant` >= 1 | `fertilizer spill` >= 1 | `gasoline spill` >= 1 | `glycol release` >= 1 | manure >= 1 | `manure spill` >= 1 | `nuclear plant shutdown` >= 1 | `paper mill` >= 1 | `power plant related` >= 1 | spill >= 1 | `styrene toxicity` >= 1 | `temp fluctuation from plant` >= 1 | `wwtp released` >= 1 | `zebra mussel treatment` >= 1 | `possible toxin` >= 1 | `possible paint` >= 1, 1, 0),
    # Infectious
    infectious = ifelse(`a.hydrophila intestine+` >= 1 | `bacterial and parasitic infection` >= 1 | `bacterial gill disease` >= 1 | `bacterial septicemia` >= 1 | `bleeding disease?` >= 1 | `carp erythrodermatitis` >= 1 | columnaris >= 1 | `columnaris disease and aermonas septicemia)` >= 1 | `columnaris with parasitic infection` >= 1 | `environmental gill disease` >= 1 | `environmental gill disease with low level of secondary parasitic infection.` >= 1 | `environmental gill disease.  ` >= 1 | `environmental gill disease; systemic bacterial infection due to a. hydrophila, p. fluorescens and external and internal parasitic infections` >= 1 | `gill parasites` >= 1 | `intralamellar henneguya` >= 1 | `opportustic bacterial infection` >= 1 | `parasite infestation and stress` >= 1 | `regressed lymphosarcoma from last year. ` >= 1 | svc >= 1 | `possible columnaris` >= 1 | `suspected disease` >= 1 | `environmental stress with columnaris` >= 1, 1, 0),
    # Summerkill
    summerkill = ifelse(summerkill >= 1 | `low 02, high temp` >= 1 | `warm temp` >= 1 | `02 supersaturation` >= 1, 1, 0),
    summerkill = ifelse(`low 02` >= 1 | `low 02, stress` >= 1 | `low o2` >= 1 | `change in temperatures` >= 1 | `thermal shock` >= 1 | `possible low 02` >= 1 && month >= 6 && month <= 10, 1, 0),
    # Winterkill
    winterkill = ifelse(winterkill >= 1 | `cold shock` >= 1 | `partial winterkill` >= 1 && month >= 11, 1, 0),
    winterkill = ifelse(winterkill >= 1 | `cold shock` >= 1 | `partial winterkill` >= 1 | `possible low 02` >= 1 && month <= 4, 1, winterkill),
    # Unknown
    unknown = ifelse(unknown >= 1 | `algal bloom` >= 1 | `environmental factor` >= 1 | pending >= 1 | stress >= 1 | u >= 1 | `low water levels, leafs` >= 1, 1, 0),
    unknown = ifelse(anthropogenic == 0 & infectious == 0 & summerkill == 0 & winterkill == 0 & unknown == 0, 1, unknown)) %>%
    dplyr::select(., c("event", "month", "year", "waterbody.name",
                     "county", "state", "long", "lat",
                     "centrarchidae", "percidae", "esocidae",
                     "ictaluridae", "cyprinidae", "salmonidae",
                     "osmeridae", "catostomidae", "sciaenidae", 
                     "lepisosteidae", "bullhead", "brown.trout",
                     "largemouth.bass", "smallmouth.bass", "minnow",
                     "cisco", "channel.catfish", "freshwater.drum",
                     "northern.pike", "rainbow.smelt", "walleye",
                     "carp", "crappie", "trout", "sunfish", "white.sucker",
                     "yellow.perch", "muskies", "bass", "stonecat",
                     "gizzard.gizzard.shad", "rockbass", "sauger", "gar",
                     "whitefish", "unknown", "summerkill", "winterkill",
                     "anthropogenic", "infectious", "cause.detail"))
...
mn.fish <- mn.fish %>%
  mutate(
    # pl2003098; Elk Lake, Grant County, MN
    lat  = ifelse(event == "pl2003098", "45.866267", lat),
    long = ifelse(event == "pl2003098", "-95.802646", long),
    # s2003062; Diamond Lake, Hennepin County, MN
    lat  = ifelse(event == "s2003062", "44.900290", lat),
    long = ifelse(event == "s2003062", "-93.269375", long),
    # s2003103; Madison Lake, Blue Earth County, MN
    lat  = ifelse(event == "s2003103", "44.192717", lat),
    long = ifelse(event == "s2003103", "-93.808331", long),
    # s2003171; Hallet's Pond, Nicolett County, MN
    lat  = ifelse(event == "s2003171", "44.338106", lat),
    long = ifelse(event == "s2003171", "-93.952646", long),
    # s2003224; Mink Lake, Wright County, MN
    lat  = ifelse(event == "s2003224", "45.272577", lat),
    long = ifelse(event == "s2003224", "-94.023599", long),
    # s2003231; Gull Lake, Cass County, MN
    lat  = ifelse(event == "s2003231", "46.456247", lat),
    long = ifelse(event == "s2003231", "-94.334078", long),
    # s2004168; Round Lake, Becker County, MN
    lat  = ifelse(event == "s2004168", "47.038514", lat),
    long = ifelse(event == "s2004168", "-95.543234", long),
    # s2005114 and s2011077; Twin Lake, Hennepin County, MN
    lat  = ifelse(event == "s2005114" | event == "s2011077", "45.045690", lat),
    long = ifelse(event == "s2005114" | event == "s2011077", "-93.340588", long),
    # s2006090; Washington Lake, Le Suerur County
    lat  = ifelse(event == "s2006090", "44.246514", lat),
    long = ifelse(event == "s2006090", "-93.868075", long),
    # s2006102; Long Lake, Hennepin County, MN
    lat  = ifelse(event == "s2006102", "44.988472", lat),
    long = ifelse(event == "s2006102", "-93.555642", long),
    # s2009170l; Twin Pond, St. Louis County, MN
    lat  = ifelse(event == "s2009170l", "46.778346", lat),
    long = ifelse(event == "s2009170l", "-92.123294", long),
    # pl2011248; Long Lake, Otter Tail County, MN
    lat  = ifelse(event == "pl2011248", "46.410217", lat),
    long = ifelse(event == "pl2011248", "-95.549014", long)) %>%
...
mn.fish.1 <- mn.fish.1 %>%
  mutate(
    ...
    centrarchidae = ifelse(event == "pl2013155(s2013141)", 0, centrarchidae),
    percidae      = ifelse(event == "pl2013155(s2013141)", 0, percidae),
    esocidae      = ifelse(event == "pl2013155(s2013141)", 0, esocidae),
    ictaluridae   = ifelse(event == "pl2013155(s2013141)", 0, ictaluridae),
    cyprinidae    = ifelse(event == "pl2013155(s2013141)", 0, cyprinidae),
    salmonidae    = ifelse(event == "pl2013155(s2013141)", 0, salmonidae),
    osmeridae     = ifelse(event == "pl2013155(s2013141)", 0, osmeridae),
    catostomidae  = ifelse(event == "pl2013155(s2013141)", 0, catostomidae),
    sciaenidae    = ifelse(event == "pl2013155(s2013141)", 0, sciaenidae),
    lepisosteidae = ifelse(event == "pl2013155(s2013141)", 0, lepisosteidae))

# Fix mn.fish.2
mn.fish.2 <- mn.fish.2 %>%
  mutate(
    # Remove s2011088; Keep s2011090
    # Causes: two winterkills
    # Taxa: different
    ictaluridae     = ifelse(event == "s2011090", 1, ictaluridae),
    bullhead        = ifelse(event == "s2011090", 1, bullhead),
    channel.catfish = ifelse(event == "s2011090", 1, channel.catfish),
    # Remove pl2007098; Keep pl2007099
    # Causes: two unknown
    # Taxa: different
    centrarchidae   = ifelse(event == "pl2007099", 1, centrarchidae),
    sunfish         = ifelse(event == "pl2007099", 1, sunfish),
    # Remove s2013138; Keep pl2013162(s2013200)
    # Causes: one unknown, one infectious = infectious
    # Taxa: different
    crappie         = ifelse(event == "pl2013162", 1, crappie),
    # Remove pl2006111; Keep s2006073
    # Causes: two unknowns
    # Taxa: different
    crappie         = ifelse(event == "s2006073", 1, crappie),
    # Remove s2003234; Keep pl2003150
    # Causes: one unknown, one anthropogenic = anthropoogenic
    # Taxa: different
    anthropogenic   = ifelse(event == "pl2003150", 1, anthropogenic),
    unknown         = ifelse(event == "pl2003150", 1, unknown),
    # Remove s2011188; Keep s2011187
    # Causes: two unknowns
    # Taxa: different
    salmonidae      = ifelse(event == "s2011187", 1, salmonidae),
    whitefish       = ifelse(event == "s2011187", 1, whitefish)) %>%
  # Removes events that were combined above after accounting for different causes and taxa
  filter(!event == "s2011188") %>%
  filter(!event == "s2003234") %>%
  filter(!event == "pl2005150") %>%
  filter(!event == "pl2006111") %>%
  filter(!event == "pl2005106") %>%
  filter(!event == "s2005118") %>%
  filter(!event == "pl2007129") %>%
  filter(!event == "s2006039") %>%
  filter(!event == "s2013138") %>%
  filter(!event == "pl2007098") %>%
  filter(!event == "s2013146") %>%
  filter(!event == "pl2007100") %>%
  filter(!event == "pl2007101") %>%
  filter(!event == "s2011088")
...
mn.fish <- mn.fish.1 %>%
  ...
  dplyr::rename(gizzard.shad = gizzard.gizzard.shad) %>%
  mutate(month = as.numeric(month),
         clupeidae = ifelse(gizzard.shad >= 1, 1, 0)) %>% # add missing family column
  # Reorganize columns 
  dplyr::select(., c(event, site, site_id, gnis_nm, lat, long, wbic,
                     month, year, waterbody.name, county, state,
                     centrarchidae, clupeidae, percidae, esocidae, ictaluridae,
                     cyprinidae, salmonidae, osmeridae, catostomidae, sciaenidae,
                     lepisosteidae, bullhead, brown.trout, largemouth.bass,
                     smallmouth.bass,  minnow, cisco, channel.catfish, freshwater.drum,
                     northern.pike, rainbow.smelt, walleye, carp, crappie, trout,
                     sunfish, white.sucker, yellow.perch, muskies, bass, stonecat,
                     gizzard.shad, rockbass, sauger, gar, whitefish, unknown, summerkill,
                     winterkill, anthropogenic, infectious, cause.detail)) %>%
  mutate_at(c(13:53), as.numeric) %>% # convert families and species to numeric
  mutate(fish_families = rowSums(.[, c(13:23)], na.rm = TRUE), # sum families
         fish_species  = rowSums(.[, c(24, 28:41, 43:48)], na.rm = TRUE), # sum species (exceptions of vague taxa below)
  ...
  mutate_at(., vars(c(13:23)), list(~ ifelse(fish_families == 0, NA, .))) %>% # ensures that, if no fish families were observed, NAs are in all the species columns to reduce the number of false negatives
  mutate_at(., vars(c(24:48)), list(~ ifelse(fish_species == 0, NA, .))) %>% # ensures that, if no fish species were observed, NAs are in all the families columns to reduce the number of false negatives
...
This reformats the WI fishkill dataset. First, this combines fishkills that occurred in the same waterbody and month into a single event as above. All taxa are organized by family and, if possible, species. Second, this associates each fishkill event with the centroid of the nearest waterbody as above. Note that a truncated version of "site_id" is already in the original WI fishkill dataset, but I add the full version for consistency with the MN data. Third, this calculates species and family richness for each fishkill event.

```{r, echo = `FALSE`}
# Save species and families
taxa <- c("sunnies", "bluegill", "crappie", "bass", "bullhead", "suckerfish",
          "trout", "gizzard.shad", "round.goby", "cisco", "pike", "snail", "muskies",
          "walleye", "carp", "sheapshead", "perch", "catfish", "sculpin", "stickleback",
          "minnow", "bigmouth.buffalo", "pumpkinseed", "drum", "darter", "dace", "chub",
          "crayfish", "redhorse", "frogs", "sunfish", "golden.shiner", "blowfin", "dogfish",
          "sturgeon", "fish_species")
...
wi.fish <- wi.fish %>%
  ...
  mutate_all(tolower) %>%
  dplyr::filter(date > "2003-01-01", date < "2013-12-31") %>%  # changed to match timespan of Phelps, removed last 6 months
  dplyr::rename(event        = fishkill.inv.seq.no,
                cause.group  = cause.category.4,
                fish_species = x..fish.species.confirmed,
                cause.detail = detail.cause.type) %>%
  ...
  dplyr::rename(anthropogenic = `anthropogenic condition`,
                infectious    = `infectious agent`) %>%
  replace_with_na_all(condition = ~.x == "unknown") %>% # replaces unknowns with NAs
  mutate_at(c(taxa), as.numeric) %>% # bluegill error is fine
  ...
  dplyr::select(., -c(1, 4, 6, 7, 9:11, 13:17, 19:21, 33, 49, 51, 57:59, 64)) %>% # removes unneccessary columns
  merge(., site.id, by = "wbic", all.x = TRUE) %>%
  mutate_at(c("event", "year", "month"), as.character) # convert event, month, and year to character
...
wi.fish[wi.fish$event == 64,   c(7:38, 47:60)][is.na(wi.fish[wi.fish$event == 64,   c(7:38, 47:60)])] = 0
wi.fish[wi.fish$event == 132,  c(7:38, 47:60)][is.na(wi.fish[wi.fish$event == 132,  c(7:38, 47:60)])] = 0
wi.fish[wi.fish$event == 3321, c(7:38, 47:60)][is.na(wi.fish[wi.fish$event == 3321, c(7:38, 47:60)])] = 0
wi.fish[wi.fish$event == 3284, c(7:38, 47:60)][is.na(wi.fish[wi.fish$event == 3284, c(7:38, 47:60)])] = 0
wi.fish[wi.fish$event == 2262, c(7:38, 47:60)][is.na(wi.fish[wi.fish$event == 2262, c(7:38, 47:60)])] = 0
wi.fish[wi.fish$event == 2259, c(7:38, 47:60)][is.na(wi.fish[wi.fish$event == 2259, c(7:38, 47:60)])] = 0
wi.fish[wi.fish$event == 2160, c(7:38, 47:60)][is.na(wi.fish[wi.fish$event == 2160, c(7:38, 47:60)])] = 0
wi.fish[wi.fish$event == 2033, c(7:38, 47:60)][is.na(wi.fish[wi.fish$event == 2033, c(7:38, 47:60)])] = 0
wi.fish[wi.fish$event == 1722, c(7:38, 47:60)][is.na(wi.fish[wi.fish$event == 1722, c(7:38, 47:60)])] = 0
wi.fish[wi.fish$event == 1508, c(7:38, 47:60)][is.na(wi.fish[wi.fish$event == 1508, c(7:38, 47:60)])] = 0
wi.fish[wi.fish$event == 1455, c(7:38, 47:60)][is.na(wi.fish[wi.fish$event == 1455, c(7:38, 47:60)])] = 0
wi.fish[wi.fish$event == 1399, c(7:38, 47:60)][is.na(wi.fish[wi.fish$event == 1399, c(7:38, 47:60)])] = 0
wi.fish[wi.fish$event == 1269, c(7:38, 47:60)][is.na(wi.fish[wi.fish$event == 1269, c(7:38, 47:60)])] = 0
wi.fish[wi.fish$event == 1136, c(8:17, 19:32, 35:38, 47:60)][is.na(wi.fish[wi.fish$event == 1136, c(8:17, 19:32, 35:38, 47:60)])] = 0
wi.fish[wi.fish$event == 107,  c(8:17, 19:32, 35:38, 47:60)][is.na(wi.fish[wi.fish$event == 107,  c(8:17, 19:32, 35:38, 47:60)])] = 0
...
# Creates columns for the combination of events in the same month and waterbody, and designates which columns (species and family) to combine. This step ensures that all taxa are accounted for in duplicate events.
wi.columns    = c("wbic", "year", "month")
wi.duplicates = c("bluegill", "crappie",
...
wi.fish <- wi.fish.1 %>%
  ...
  mutate(month = as.numeric(month),
         fish_families = rowSums(.[, c(43:56)]), # sum families
         fish_species  = rowSums(.[, c(10:34)]), # sum species (exceptions below)
         fish_species = ifelse(bluegill >= 1 | crappie >= 1 | bass >= 1 & !is.na(bluegill) & !is.na(crappie) & !is.na(bass), (fish_species + 1), fish_species)) %>%
  mutate_all(na_if,"") %>% # replaces empty cells with NAs
  mutate_at(., vars(c(43:56)), list(~ ifelse(fish_families == 0, NA, .))) %>% # ensures that, if no fish families were observed, NAs are in all the species columns to reduce the number of false negatives
  mutate_at(., vars(c(10:34)), list(~ ifelse(fish_species == 0, NA, .))) %>% # ensures that, if no fish species were observed, NAs are in all the families columns to reduce the number of false negatives
  mutate(fish_families = ifelse(fish_families == 0, NA, fish_families),
...
fish <- mn.fish %>%
  ...
  mutate(year      = as.numeric(year),
         ...
         warm_temp = ifelse(bullhead >= 1 | largemouth.bass >= 1 | smallmouth.bass >= 1 | channel.catfish >= 1 | freshwater.drum >= 1 | sunfish >= 1 | bass >= 1 | freshwater.drum >= 1 | stonecat >= 1 | drum >= 1 | rockbass >= 1 | gar >= 1 | gizzard.shad >= 1 | sheapshead >= 1 | catfish >= 1 | bigmouth.buffalo >= 1 | pumpkinseed >= 1 | golden.shiner >= 1 | blowfin >= 1 | dogfish >= 1 | minnow >= 1 | darter >= 1 | perch >= 1 | suckerfish >= 1 | crappie >= 1 & !is.na(bullhead) & !is.na(largemouth.bass) & !is.na(smallmouth.bass) & !is.na(channel.catfish) & !is.na(freshwater.drum) & !is.na(sunfish) & !is.na(bass) & !is.na(freshwater.drum) & !is.na(stonecat) & !is.na(drum) & !is.na(rockbass) & !is.na(gar) & !is.na(gizzard.shad) & !is.na(sheapshead) & !is.na(catfish) & !is.na(bigmouth.buffalo) & !is.na(pumpkinseed) & !is.na(golden.shiner) & !is.na(blowfin) & !is.na(dogfish) & !is.na(minnow) & !is.na(darter) & !is.na(perch) & !is.na(suckerfish) & !is.na(crappie), 1, 0), # Warm in Lyons et al., 2019
...
### Step 5: Add water temperature to historical dataset
...
hist.water.annual <- hist.water %>%
  ...
  ungroup() %>%
  dplyr::rename(ice_duration = ice_duration_days,
...
# Correlation matrix of water temperature metrics
cor(x = subset(hist.water.month, select = c(max_surf, mean_surf, mean_bot)))
...
water.pca.prop <- water.pr$sdev / sum(water.pr$sdev)
round(water.pca.prop, 2)
...
rm(hist.water.annual, hist.water.month,
   water.pr, water_pca, water.pca.prop)
```

### Step 6a: Add air temperature data (NOAA) to historical dataset

This was my first attempt at obtaining historical air temperature data (2003-2014). This dataset includes daily air temperature data for all Minnesota and Wisconsin weather stations from NOAA (https://www.ncdc.noaa.gov/cdo-web/search?datasetid=GHCND). However, there are many missing data for both states, and so I decided to use air temperature estimates from PRISM instead (Step 7b). This is all hashed out but retained in case anyone finds it helpful. For this particular step, I first merged all air temperature data using BASH. I then reformatted the date columns across datasets and calculated the monthly and annual averages of maximum, mean, and minimum air temperatures. Weather stations were then associated with the nearest waterbody centroid.

```{bash eval = `FALSE`}
#cd /Users/simontye/Research/Projects/MME_Temp/2020_MME_Temp/data/raw/temp
...
### Step 6b: Add air temperature (PRISM) to historical dataset
...
"data/raw/temp_air/2003_prism/PRISM_tmean_stable_4km_200301_201312_#.csv". Second, I manually merged these files into "data/raw/prism_1.csv" and "data/raw/prism_2.csv". This required two files because Excel doesn't enjoy files with >1,000,000 rows.
...
df.historical <- df.historical %>%
  ...
  # Calculate quadratic temp
  mutate(air_quad_temp = mean_air^2)

# Correlation matrix of air temperature metrics
cor(x = subset(df.historical, select = c(max_air, mean_air, min_air)))
...
# Calculate proportion of variance explained
air.pca.prop <- air.pr$sdev / sum(air.pr$sdev)
round(air.pca.prop, 2)
...
### Step 7: Add geographic, census, and ecoregion data to historical dataset

This adds geographic (county, state), 2010 US census, and ecoregion data to the historical dataset. First, I used the waterbody centroid coordinates to determine the county and state for each locality. Second, I used these geographic identifiers to associate county population estimates (https://www.census.gov/geo/maps-data/data/tiger-data.html). Third, I used the waterbody localities to determine encompassing ecoregion polygons.

```{r echo = `FALSE`}
# Set working directory
setwd("/Users/simontye/Research/Projects/MME_Temp/2020_MME_Temp/data/raw")

# Load census data
census.wi <- readOGR(dsn = path.expand("census"), layer = 'tabblock2010_55_pophu')
census.mn <- readOGR(dsn = path.expand("census"), layer = 'tabblock2010_27_pophu')
...
ecoregions <- full_join(ecoregions.mn, ecoregions.wi) %>%
  ...
  dplyr::rename(l4_code = us_l4code,
                l4_name = us_l4name,
                state   = state_name) %>%
...
df.historical <- df.historical %>%
  ...
  mutate(l1_code = ifelse(l3_code == "51" | l3_code == "52" | l3_code == "53" | l3_code == "54", "8",
                          ifelse(l3_code == "46" | l3_code == "47" | l3_code == "48", "5",
                                 ifelse(l3_code == "49" | l3_code == "50", "9", NA))))
# Load spatial data
...
# Calculate z-scores
hist.z <- df.historical %>%
  group_by(site_id, month) %>%
  # Calculate new z-scorses
  dplyr::summarize(
    # Monthly means
    month_max_surf  = mean(max_surf),
    month_mean_surf = mean(mean_surf),
    month_max_bot   = mean(max_bot),
    month_mean_bot  = mean(mean_bot),
    month_max_air   = mean(max_air),
    month_mean_air  = mean(mean_air),
    month_min_air   = mean(min_air),
    # Monthly standard deviations
    sd_max_surf  = sd(max_surf),
    sd_mean_surf = sd(mean_surf),
    sd_max_bot   = sd(max_bot),
    sd_mean_bot  = sd(mean_bot),
    sd_max_air   = sd(max_air),
    sd_mean_air  = sd(mean_air),
    sd_min_air   = sd(min_air)) %>%
  mutate(month = as.numeric(month))

# Add new z-scores to historical data
df.historical <- df.historical %>%
  left_join(., hist.z, by = c("site_id", "month")) %>%
  mutate(max_surf_z  = ((max_surf - month_max_surf) / sd_max_surf),
         mean_surf_z = ((mean_surf - month_mean_surf) / sd_mean_surf),
         max_bot_z   = ((max_bot - month_max_bot) / sd_max_bot),
         mean_bot_z  = ((mean_bot - month_mean_bot) / sd_mean_bot),
         max_air_z   = ((max_air - month_max_air) / sd_max_air),
         mean_air_z  = ((mean_air - month_mean_air) / sd_mean_air),
         min_air_z   = ((min_air - month_min_air) / sd_min_air)) %>%
  mutate(month_max_surf  = NULL,
         month_mean_surf = NULL,
         month_max_bot   = NULL,
         month_mean_bot  = NULL,
         month_max_air   = NULL,
         month_mean_air  = NULL,
...
# Verify thermal categories
df.historical <- df.historical %>%
  mutate_at(c("centrarchidae"), ~replace(., is.na(.), 0)) %>%
  mutate_at(c("percidae"), ~replace(., is.na(.), 0)) %>%
  mutate_at(c("esocidae"), ~replace(., is.na(.), 0)) %>%
  mutate_at(c("ictaluridae"), ~replace(., is.na(.), 0)) %>%
  mutate_at(c("cyprinidae"), ~replace(., is.na(.), 0)) %>%
  mutate_at(c("warm_temp"), ~replace(., is.na(.), 0))

# Verify thermal categories
df.historical <- df.historical %>%
  mutate(warm_temp = ifelse(amiidae >= 1 | catostomidae >= 1 | centrarchidae >= 1 | cyprinidae >= 1 | esocidae >= 1 | ictaluridae >= 1 | lepisosteidae >= 1 | percidae >= 1 | sciaenidae >= 1, 1, 0),
         cool_temp = ifelse(clupeidae >= 1 | gasterosteidae >= 1 , 1, 0),
         cold_temp = ifelse(cottoidae >= 1 | salmonidae >= 1, 1, 0),
         unknown_temp = ifelse(acipenseridae >= 1 | gobiidae >= 1 | osmeridae >= 1, 1, 0))

# Add WBIC
df.historical <- df.historical %>%
  mutate(wbic = NULL) %>%
  left_join(., site.id[c("site_id", "wbic")], by = c("site_id")) %>%
  distinct(.)
```

### Step 8: Add lake size and productivity data

This adds lake size (km^2) and average secchi depth (m) for lakes across Wisconsin and Minnesota.

```{r, echo = `FALSE`}
# Load WI data
wi.size <- read.csv("data/raw/lake_size/fsh10228-sup-0002-dataset2.csv", sep = ",", head = TRUE)
wi.prod <- readxl::read_excel("data/raw/productivity/fsh10228-sup-0005-dataset5.xlsx")
wi.site <- read.csv(file = "data/raw/sites/WI_Site.csv", head = TRUE, sep = ",")
site.id <- read.csv(file = "data/raw/sites/Site_ID.csv", head = TRUE, sep = ",")
...
# Locations from historical dataset
df.sites <- df.historical %>%
  dplyr::select(., c(site_id)) %>%
  left_join(., site.id, by = c("site_id")) %>%
  mutate(x = NULL) %>%
  distinct(.)

# Reformat lake size data
wi.size <- wi.size %>%
  rename_all(tolower) %>%
  mutate_all(tolower) %>%
  dplyr::rename(size_acres      = official_size_acres,
                watershed_acres = watershed_area_acres) %>%
  dplyr::select(., c(wbic, size_acres, watershed_acres, max_depth_ft)) %>%
  # Remove text and reformat
  mutate(watershed_acres = gsub(" sq miles", "", watershed_acres)) %>%
  mutate(watershed_acres = as.numeric(watershed_acres),
         size_acres      = as.numeric(size_acres),
         max_depth_ft    = as.numeric(max_depth_ft)) %>%
  # Change imperial to metric
  mutate(size_km2      = size_acres * 0.0040468564224) %>%
  mutate(watershed_km2 = watershed_acres * 0.0040468564224) %>%
  mutate(max_depth_m   = max_depth_ft / 3.28084) %>%
  dplyr::select(., c(wbic, size_km2)) %>%
  filter(!is.na(size_km2))

# Reformat productivity data
wi.prod <- wi.prod %>%
  rename_all(tolower) %>%
  mutate_all(tolower) %>%
  dplyr::rename(secchi_m = `secchi (m)`) %>%
  mutate(secchi_m = as.numeric(secchi_m)) %>%
  # Filter to study period
  filter(year >= 2003) %>%
  filter(year <= 2013) %>%
  # Summarize by lake
  ungroup(.) %>%
  dplyr::select(., c(wbic, secchi_m)) %>%
  group_by(wbic) %>%
  dplyr::summarize(secchi_m = mean(secchi_m, na.rm = TRUE))
  
# Merge WI data
wi.data <- wi.size %>%
  left_join(., wi.prod, by = c("wbic")) %>%
  filter(!is.na(secchi_m))
```

```{r, echo = `FALSE`}
# Load MN data
mn.prod  <- st_read("data/raw/productivity/lakesmnwq_1975_2008_fill1975-epsg4326/MNWQ_1975_2008_fill1975-epsg4326.shp")
mn.lakes <- read.csv(file = "data/raw/lake_identifiers/MN_nhd2dowlknum.csv", head = TRUE, sep = ",")

# Reformat productivity data
mn.prod <- mn.prod %>%
  dplyr::select(., c(RNAME_1,
                     X_UTM, Y_UTM,
                     PolyAcres, unmlknum, dowlknum_1,
                     X1975MEAN, X1985MEAN, X1990MEAN,
                     X1995MEAN, X2000MEAN,
                     X2005MEAN, X2008MEAN)) %>%
  dplyr::rename(lake_name = RNAME_1,
                x_utm            = X_UTM,
                y_utm            = Y_UTM,
                size_acres       = PolyAcres,
                secchi_mean_1975 = X1975MEAN,
                secchi_mean_1985 = X1985MEAN,
                secchi_mean_1990 = X1990MEAN,
                secchi_mean_1995 = X1995MEAN,
                secchi_mean_2000 = X2000MEAN,
                secchi_mean_2005 = X2005MEAN,
                secchi_mean_2008 = X2008MEAN,
                dowlknum         = dowlknum_1)

# Calculate mean secchi depth across same timespan as fishkills
mn.prod.final <- df.sites %>%
  left_join(., mn.lakes, by = c("site_id")) %>%
  dplyr::select(., c("site_id", "dowlknum")) %>%
  left_join(., mn.prod, by = c("dowlknum")) %>%
  dplyr::select(., c("site_id", "dowlknum",
                     "secchi_mean_2000",
                     "secchi_mean_2005",
                     "secchi_mean_2008")) %>%
  group_by(site_id, dowlknum) %>%
  dplyr::summarize(secchi_mean_2000 = mean(secchi_mean_2000, na.rm = TRUE),
                   secchi_mean_2005 = mean(secchi_mean_2005, na.rm = TRUE),
                   secchi_mean_2008 = mean(secchi_mean_2008, na.rm = TRUE)) %>%
  dplyr::summarize(secchi_m = mean(c(secchi_mean_2000, secchi_mean_2005, secchi_mean_2008), na.rm = TRUE)) %>%
  # Replace NaN w/ NA and omit
  mutate(secchi_m = ifelse(secchi_m == "NaN", NA, secchi_m)) %>%
  na.omit(.)

# Calculate lake size by getting mean of each bay and combining bays (different dowlknum for same site_id)
mn.size.final <- df.sites %>%
  left_join(., mn.lakes, by = c("site_id")) %>%
  dplyr::select(., c("site_id", "dowlknum")) %>%
  left_join(., mn.prod, by = c("dowlknum")) %>%
  dplyr::select(., c("site_id", "dowlknum",
                     "size_acres")) %>%
  # Replace NaN w/ NA and omit
  mutate(size_acres = ifelse(size_acres == "NaN", NA, size_acres)) %>%
  distinct(.) %>%
  group_by(site_id, dowlknum) %>%
  dplyr::summarize(size_acres = mean(size_acres, na.rm = TRUE)) %>%
  distinct(.) %>%
  ungroup(.) %>%
  group_by(site_id) %>%
  dplyr::summarize(size_acres = sum(size_acres, na.rm = TRUE)) %>%
  filter(size_acres != 0) %>%
  # Convert to km2
  mutate(size_km2 = size_acres * 0.0040468564224) %>%
  mutate(size_acres = NULL)

# Combine MN data
mn.data <- merge(mn.prod.final, mn.size.final, all = TRUE)

# Add new covariates and reduce dataset
df.historical <- df.sites %>%
  left_join(., mn.data) %>%
  left_join(., wi.data) %>%
  dplyr::select(., c("site_id", "secchi_m", "size_km2")) %>%
  na.omit(.) %>%
  left_join(df.historical, ., by = c("site_id")) %>%
  distinct(.) %>%
  filter(!is.na(secchi_m))
...
rm(fish, df.historical,
   ...
   localities, spatial,
   mn.size.final,
   mn.prod, mn.prod.final)
```

### Step 8: Add water temperature to future dataset
...
future.water <- read_tsv("data/raw/temp_water/ACCESS_thermal_metrics.tsv") %>%
  inner_join(site.id, by = "site_id")
...
### Step 9: Add air temperature (NOAA) to future dataset
...
localities.sp <- SpatialPoints(localities[, c("lat", "long")])
site.sp       <- SpatialPoints(site.id[, c("lat", "long")])
...
df.future <- df.future %>%
  ...
  mutate(id = NULL, # remove temporary unique identifier
        air_quad_temp = mean_air^2) %>%
  subset(., !is.na(mean_air)) %>% # remove rows with missing data
...
### Step 10: Add geographic, census, ecoregion, productivity, and lake size to future dataset
...
# Calculate new z-scores for mid century data
future.mid.z <- df.future %>%
  filter(year < 2075) %>%
  group_by(site_id, month) %>%
  # Calculate new z-scorses
  dplyr::summarize(
...
# Separate time periods to add appropriate z-scores
df.future.mid <- df.future %>%
  filter(year < 2075) %>%
  left_join(., future.mid.z, by = c("site_id", "month"))

df.future.late <- df.future %>%
  filter(year > 2075) %>%
  left_join(., future.late.z, by = c("site_id", "month"))
...
This creates a snowfall dataset for the entire geographic range of Winslow et al. (2017).
...
tidy_snow <- function(path) {
  ...
  data_output <- data %>%
    group_by(long_round, lat_round) %>%
    dplyr::summarise(snow = mean(snow))
