...
- `census_data`:       2010 US census data
- `lake_shapes`:       GIS polygons of waterbodies in the upper midwest
- `localities`:        List of csv files used to obtain historical air temperature estimates
- `temp_air`:          Historical and future water temperature estimates 
- `wisconsin_outline`: GIS polygons of Wisconsin
...
```{r: Load Minnesota fishkill data}
...
# Adds localities of the centroid of waterbodies for events that had a location name but no coordinates These coordinates were found in Google Earth and saved as "data/raw/mme_missing.kml". For transparency, below are each of the events, location names, and coordinates. I was unable to find coordinates for all of the missing locations. The modified MN dataset is exported at the end of these steps to see which do and do not have coordinates. Note that 1) event s2005163 is Rice Lake, Faribault County, but there are 2 Rice Lakes in that county via Google Earth and 2) the coordinates were changed for Long Lake, Otter Tail County, because the coordinates were off.
...
# Writes csv of modified MN dataset for Phelps to review
write.csv(mn.fish, "data/raw/mn_phelps.csv", row.names = TRUE)
...
mn.fish <- mn.fish %>%
  mutate(lat  = as.numeric(lat), # NAs that were "introduced" were empty cells
         long = as.numeric(long),
         site = c()) %>% # NAs that were "introduced" were empty cells
  subset(., !is.na(lat))
...
# Adjust site number for waterbody association
site.id$site <- c(1:10962)
...
# Causes: two infectious, one unknonwn = infectious
# Taxa: different
...
mn.fish <- mn.fish.1 %>%
  ...
  mutate(month = as.numeric(month),
         clupeidae = ifelse(gizzard.gizzard.shad >= 1, 1, 0)) %>% # add missing family column
...
```{r: Load Wisconsin fishkill data}
...
wi.fish <- wi.fish %>%
  ...
  mutate(month = fct_collapse(month,"12"  = "dec",
                                    "1"   = "jan",
                                    "2"   = "feb",
                                    "3"   = "mar",
                                    "4"   = "apr",
  ...
  dplyr::select(., -c(1, 4, 6, 7, 9:11, 13:18, 20:21, 33, 49, 51, 57:59, 63)) %>% # removes non-fish taxa and unneccessary columns
  merge(., site.id, by = "wbic", all.x = TRUE) %>%
  mutate_at(c(4:6), as.character) %>% # convert event, month, and year to character
  mutate_at(c(7:38, 46:59), as.numeric) # convert families and species to numeric
...
# Event: 64
# Row: 552
wi.fish[552, c(7:38, 46:59)][is.na(wi.fish[552, c(7:38, 46:59)])] = 0

# Event: 616
# Row: 132
wi.fish[132, c(7:38, 46:59)][is.na(wi.fish[132, c(7:38, 46:59)])] = 0

# Event: 3321
# Row: 274
wi.fish[274, c(7:38, 46:59)][is.na(wi.fish[274, c(7:38, 46:59)])] = 0

# Event: 3284
# Row: 463
wi.fish[463, c(7:38, 46:59)][is.na(wi.fish[463, c(7:38, 46:59)])] = 0

# Event: 2262
# Row: 302
wi.fish[302, c(7:38, 46:59)][is.na(wi.fish[302, c(7:38, 46:59)])] = 0

# Event: 2259
# Row: 552
wi.fish[200, c(7:38, 46:59)][is.na(wi.fish[200, c(7:38, 46:59)])] = 0

# Event: 2160
# Row: 552
wi.fish[259, c(7:38, 46:59)][is.na(wi.fish[259, c(7:38, 46:59)])] = 0

# Event: 2033
# Row: 552
wi.fish[260, c(7:38, 46:59)][is.na(wi.fish[260, c(7:38, 46:59)])] = 0

# Event: 1722
# Row: 552
wi.fish[194, c(7:38, 46:59)][is.na(wi.fish[194, c(7:38, 46:59)])] = 0

# Event: 1508
# Row: 552
wi.fish[193, c(7:38, 46:59)][is.na(wi.fish[193, c(7:38, 46:59)])] = 0

# Event: 1455
# Row: 552
wi.fish[413, c(7:38, 46:59)][is.na(wi.fish[413, c(7:38, 46:59)])] = 0

# Event: 1399
# Row: 552
wi.fish[206, c(7:38, 46:59)][is.na(wi.fish[206, c(7:38, 46:59)])] = 0

# Event: 1269
# Row: 552
wi.fish[458, c(7:38, 46:59)][is.na(wi.fish[458, c(7:38, 46:59)])] = 0

# Event: 1136
# Row: 552
wi.fish[288, c(7:38, 46:59)][is.na(wi.fish[288, c(7:38, 46:59)])] = 0

# Event: 107
# Row: 552
wi.fish[395, c(7:38, 46:59)][is.na(wi.fish[395, c(7:38, 46:59)])] = 0
...
### Step 4: Format final fishkill dataset
...
```{r: Format combined fishkill dataset}
...
fish <- mn.fish %>%
  ...
  mutate(year = as.numeric(year),
         ...
         warm_temp = ifelse(bullhead >= 1 | largemouth.bass >= 1 | smallmouth.bass >= 1 | channel.catfish >= 1 | freshwater.drum >= 1 | redhorse >= 1 | sunfish >= 1 | bass >= 1 | freshwater.drum >= 1 | stonecat >= 1 | gizzard.gizzard.shad >= 1 | drum >= 1 | rockbass >= 1 | gar >= 1 | gizzard.shad >= 1 | sheapshead >= 1 | catfish >= 1 | bigmouth.buffalo >= 1 | pumpkinseed >= 1 | golden.shiner >= 1 | blowfin >= 1 | dogfish >= 1 | minnow >= 1 | darter >= 1 | perch >= 1 | suckerfish >= 1 | crappie >= 1 & !is.na(bullhead) & !is.na(largemouth.bass) & !is.na(smallmouth.bass) & !is.na(channel.catfish) & !is.na(freshwater.drum) & !is.na(redhorse) & !is.na(sunfish) & !is.na(bass) & !is.na(freshwater.drum) & !is.na(stonecat) & !is.na(gizzard.gizzard.shad) & !is.na(drum) & !is.na(rockbass) & !is.na(gar) & !is.na(gizzard.shad) & !is.na(sheapshead) & !is.na(catfish) & !is.na(bigmouth.buffalo) & !is.na(pumpkinseed) & !is.na(golden.shiner) & !is.na(blowfin) & !is.na(dogfish) & !is.na(minnow) & !is.na(darter) & !is.na(perch) & !is.na(suckerfish) & !is.na(crappie), 1, 0), # Warm in Lyons et al., 2019
  ...
  dplyr::select(., -c(10, 13:30, 36, 39:77, 79:83)) %>% # removes fish families and species
  .[with(., order(.$fish_families)), ] # sorts by presence of taxa observations

# Replaces NAs with 0s for events that documented at least one taxa. The last function orders the rows by number of documented taxa to change the top rows that included at least one taxa observation.
fish[1:321, 20:23][is.na(fish[1:321, 20:23])] = 0
...
### Step 6: Add historical water temperature data
...
### Step 7a: Add historical air temperature data (NOAA)
...
### Step 7b: Add historical air temperature data (PRISM)
...
# Subset sites from Winslow et al. (2017)
sites <- subset(hist.water, select = c(site_id))

# Retain unique sites
sites <- unique(sites)

# Subset site coordinates from site_id
localities <- subset(site.id, select = c(site_id, lat, long))

# Retain unique localities
...
# Remove empty rows between localities (PRISM default). Note: only works when perforrmed separately, not after joining both 
...
# Add historical air temperature
df.historical <- localities %>%
  subset(., select = c(site_id, id)) %>%
  mutate(id = as.character(id)) %>%
  full_join(hist.air, ., by = "id") %>%
  full_join(df.historical, ., by = c("site_id", "year", "month")) %>%
  mutate(lat.x  = NULL,
         long.x = NULL,
         id     = NULL) %>%
  rename(lat    = lat.y,
  ...
  fill(c(lat, long, gnis_nm), .direction = "downup") %>% # fills in waterbody characteristics by site_id
  dplyr::ungroup()
...
### Step 8: Add geographic and census data

This adds geographic (county, state) and 2010 US census data to the historical dataset. First, I used the waterbody centroid coordinates to determine the county and state for each locality. Second, I merged spatial covariates for each waterbody into the dataset. Third, I added 2010 census data from the US Census Bureau (https://www.census.gov/geo/maps-data/data/tiger-data.html).

```{r: Add geographic and census data}
# Isolate coordinates to determine county and state of each waterbody
coordinates <- subset(df.historical, select = c("long", "lat"))
...
# Finds county and state for each set of coordinates
county.state <- latlong2county(coordinates)
county.state <- as.vector(county.state)

# Add counties and states
df.historical$state <- county.state

# Remove unnecessary dataframes
rm(coordinates, localities, sites, county.state)

# Create county and state columns from date column
df.historical[50:51] <- do.call('rbind', strsplit(as.character(df.historical$state), ','))

# Rename and reformat year and month columns
df.historical <- df.historical %>%
  rename(county  = V2) %>%
  mutate(state = ifelse(state == "minnesota", "mn", state),
         state = ifelse(state == "wisconsin", "wi", state),
         state = ifelse(state == "michigan",  "mi", state))

# Fill empty cells for fish_families, fish_species, and fishkill causes with 0s if there was not a fishkill event
df.historical[, 27:37][is.na(df.historical[, 27:37])] = 0

# Load spatial data
spatial <- readOGR(dsn = path.expand("data/raw/lake_shapes"), layer = "model_lakes")
spatial.df <- coordinates(spatial) %>%
  as.data.frame() %>%
...
# Merge spatial data
df.historical <- df.historical %>%
  inner_join(spatial.df, by = "site_id") %>%
  mutate(long.x = NULL,
         lat.x  = NULL) %>%
  rename(long  = long.y,
         lat   = lat.y)
...
df.historical <- df.historical %>%
  mutate(lon_round = round(long, 1),
         lat_round  = round(lat, 1)) %>%
  left_join(census.df, by = c("lon_round", "lat_round")) %>%
...
df.future <- future.water.monthly %>%
  ...
  left_join(spatial.df, by = "site_id") %>%
  mutate(lon_round = round(long, 1),
         lat_round = round(lat, 1)) %>%
  left_join(census.df, by = c("lon_round", "lat_round")) %>%
  dplyr::select(-lon_round, -lat_round) %>%
...
rm(water.pr, water_pca, future.water,
   ...
   census.df, spatial.df)
```

### Step 10: Add future air temperature data
...
# Remove population (too many NAs) and missing air temperature data (a few NAs)
df.future <- df.future %>%
  dplyr::select(., -c("population")) %>%
  subset(., !is.na(mean_air)) %>%
...
# Isolate coordinates to determine county and state of each waterbody
coordinates <- subset(df.future, select = c("long", "lat"))

# Finds county and state for each set of coordinates
county.state <- latlong2county(coordinates)

# Add counties and states
df.future$state <- as.vector(county.state)

# Remove unnecessary dataframes
rm(coordinates, county.state)

# Create county and state columns from date column
df.future[38:39] <- do.call('rbind', strsplit(as.character(df.future$state), ','))
...
# Save processed data
write_csv(df.future, "data/processed/df_future.csv")

# Remove unneccessary dataframes
rm(df.future, site.id, latlong2county)
```

### Step 11: Add snowfall data

This creates a snowfall dataset for the entire geographic range of the Winslow et al. (2017) dataset, which encompasses WI, MN, and MI. After performing this step, the historical (2003-2014) and future (2041-2059; 2081-2099) datasets are fully processed and ready for modeling.
