...
model_data_month <- read_csv("model_data_month.csv",
...
# Set seed
set.seed(998)
in_training <- createDataPartition(model_data_month$summerkill, p = .75, list = FALSE)

# Partition data
training <- model_data_month  %>%
  slice(in_training)
testing  <- model_data_month %>%
  slice(-in_training)
