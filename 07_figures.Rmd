...
# Load probabilities
prob.hist.a2   <- fread("data/processed/ridge_a2_historical.csv")
prob.hist.w1   <- fread("data/processed/ridge_w1_historical.csv")
prob.future.a2 <- fread("data/processed/ridge_a2_future.csv")
prob.future.w1 <- fread("data/processed/ridge_w1_future.csv")
...
df.historical <- df.historical %>%
  ...
  mutate(summerkill = ifelse(summerkill == 1, 1, 0)) %>%
  mutate(summerkill = ifelse(is.na(summerkill) == TRUE, 0, summerkill)) %>%
  mutate(summerkill = as.numeric(summerkill))
...
boxplot.raw.summer <- df.historical %>%
  ...
  dplyr::mutate(fishkill_cause = ifelse(is.na(fishkill_cause), "Summer non-event", fishkill_cause)) %>%
  ...
  dplyr::rename("Air"           = mean_air,
                ...
                "Deep water"    = mean_bot) %>%
  reshape2::melt(., id.vars = c("fishkill_cause"),
                 measure.vars = c("Air", "Surface water", "Deep water")) %>%
  dplyr::rename(`Elevation`             = variable,
                `Mean temperature (ºC)` = value) %>%
  filter(fishkill_cause == "Summerkill" | fishkill_cause == "Infectious" | fishkill_cause == "Summer non-event") %>%
  mutate(fishkill_cause = as.factor(as.character(fishkill_cause)))

# Create boxplot df for raw temps
boxplot.raw.winter <- df.historical %>%
  ...
  dplyr::mutate(fishkill_cause = ifelse(is.na(fishkill_cause), "Winter non-event", fishkill_cause)) %>%
  dplyr::mutate(fishkill       = as.factor(fishkill),
                fishkill_cause = as.factor(fishkill_cause)) %>%
...
# Create boxplot df for raw temps
boxplot.z.summer <- df.historical %>%
  filter(month %in% c("6", "7", "8")) %>%
  dplyr::mutate(fishkill = ifelse(anthropogenic == 1 | infectious == 1 | summerkill == 1 | winterkill == 1 | unknown == 1, 1, 0)) %>%
  dplyr::mutate(fishkill_cause = ifelse(anthropogenic == 1, "Anthropogenic",
                                        ifelse(infectious == 1, "Infectious",
...
boxplot.z.winter <- df.historical %>%
  filter(month %in% c("12", "1", "2")) %>%
  dplyr::mutate(fishkill = ifelse(anthropogenic == 1 | infectious == 1 | summerkill == 1 | winterkill == 1 | unknown == 1, 1, 0)) %>%
  dplyr::mutate(fishkill_cause = ifelse(anthropogenic == 1, "Anthropogenic",
                                        ifelse(infectious == 1, "Infectious",
 ...
 # Reorder levels
boxplot.raw.summer$Elevation <- factor(boxplot.raw.summer$Elevation,
...
boxplot.z.summer$Elevation <- factor(boxplot.z.summer$Elevation,
                                     levels = c("Deep water", "Surface water", "Air"))
boxplot.z.summer$fishkill_cause <- factor(boxplot.z.summer$fishkill_cause,
                                          levels = c("Summerkill", "Infectious", "Summer non-event"))

# Reorder levels
boxplot.raw.winter$Elevation      <- factor(boxplot.raw.winter$Elevation,
                                            levels = c("Deep water", "Surface water", "Air"))
boxplot.raw.winter$fishkill_cause <- factor(boxplot.raw.winter$fishkill_cause,
                                            levels = c("Winterkill", "Winter non-event"))
boxplot.z.winter$Elevation      <- factor(boxplot.z.winter$Elevation,
                                          levels = c("Deep water", "Surface water", "Air"))
...
causes.state <- df.historical %>%
  ...
  dplyr::summarize(Anthropogenic = sum(anthropogenic, na.rm = TRUE),
                   ...
                   Summerkill    = sum(summerkill, na.rm = TRUE),
...
ggplot(data = states,
  ...
  geom_polygon(data = counties,
               aes(x = long, y = lat, group = group),
               alpha = 0.1, color = "darkgray", size = 0.25) +
  geom_polygon(data = states, color = "darkgray", fill = NA, size = 1) +
  # Lakes
  #geom_point(data = lakes, aes(x = long, y = lat, group = state),
  #           color = "black", fill = "black",
  #           size = 0.6, shape = 21, stroke = 0) +
  # Fishkills
  geom_jitter(data = fishkills,
              aes(x = long, y = lat, group = state, fill = fishkill_cause),
              size = 2, shape = 21, stroke = 0.25,
...
ggplot() +
  ...
  labs(x ="Cause", y = "Mortality events") +
  pretty
...
# Save figure
pdf("figures/fig_2_boxplots.pdf", width = 8, height = 6) 
...
```{r, format, echo = `FALSE`}
# Reformat response variable
df.historical2 <- df.historical2 %>%
  mutate(summerkill = ifelse(summerkill == "neg", 0, 1)) %>%
  mutate(summerkill = as.numeric(summerkill))

# Historical air model
h.a2 <- glm(summerkill ~ lat + long + season + population +
            max_air + mean_air + min_air + max_air_z + mean_air_z + min_air_z +
            precip + ice_duration + secchi_m + size_km2,
            data   = df.historical2,
            family = binomial)
...
# Air projections
p.h.a2 <- predict(object  = h.a2, 
                  newdata = df.historical2, 
                  type    = "response")
...
```{r, format, echo = `FALSE`}
# Future air model
f.a2 <- glm(summerkill ~ lat + long + season + population +
            max_air + mean_air + min_air + max_air_z + mean_air_z + min_air_z +
            precip + secchi_m + size_km2,
            data   = df.historical2,
            family = binomial)
...
```{r, format, echo = `FALSE`}
# Air: early 21st
prob.1 <- df.historical2 %>%
  # Add probabilities
  mutate(prob_air   = as.character(NA),
         prob_water = as.character(NA)) %>%
  mutate(prob_no_air      = 1 - p.h.a2,
         prob_no_water    = 1 - p.h.w1,
         generation    = "Early 21st Century") %>%
  group_by(lat, long) %>%
  dplyr::summarise(prob_air   = (1 - prod(prob_no_air)),
...
prob.2 <- df.future %>%
  ...
  mutate(prob_air   = as.character(NA),
         prob_water = as.character(NA)) %>%
  mutate(prob_no_air      = 1 - p.f.a2,
         prob_no_water    = 1 - p.f.w1,
...
time.1 <- ggplot() +
  ...
  geom_path(p.time.1, mapping = aes(x = year, y = water_smooth),
            color = "#9ecae1") +
  # Air
  ...
  geom_path(p.time.1, mapping = aes(x = year, y = air_smooth),
            color = "indianred2") +
  # Observed
  geom_point(real.hist, mapping = aes(x = year, y = summerkill),
             color = "mediumpurple2", alpha = 0.8, size = 1.25) +
  geom_path(real.hist, mapping = aes(x = year, y = summerkill_smooth),
...
time.2 <- ggplot() +
  ...
  geom_point(p.time.2, mapping = aes(x = year, y = water_smooth),
             color = "#9ecae1", alpha = 0.8, size = 1.25) +
  geom_path(p.time.2, mapping = aes(x = year, y = water_smooth),
            color = "#9ecae1") +
  # Air
  ...
  geom_path(p.time.2, mapping = aes(x = year, y = air_smooth),
            color = "indianred2") +
  # y-axis
  scale_y_continuous(breaks = c(0, 100, 200, 300),
                     labels = c("", "", "", ""),
...
plot.air.1 <- ggplot() +
  ...
  scale_fill_gradientn(limits = c(-0.04, 1.17),
                       colors = c("lightblue1", "white", "lightyellow1",
...
plot.water.1 <- ggplot() +
  ...
  geom_point(filter(fishkills, summerkill >= 1),
             ...
             size = 1.5, shape = 21, stroke = 0.25, color = "black", fill = "mediumpurple2") +
  scale_y_continuous(breaks = c(43, 45, 47, 49)) +
  scale_fill_gradientn(limits = c(-0.11, 0.80),
                       colors = c("lightblue1", "white", "lightyellow1",
                                  "lightgoldenrod1", "lightgoldenrod2",
                                  "gold", "gold1", "gold2",
                                  "orange1", "sandybrown", "orange2",
...
plot.water.2 <- ggplot() +
  ...
  geom_polygon(states, mapping = aes(x = long, y = lat, group = group),
               fill = NA, size = 0.7, color = "black") +
  scale_y_continuous(breaks = c(43, 45, 47, 49)) +
  scale_fill_gradientn(limits = c(-0.11, 0.8),
                       colors = c("lightblue1", "white", "lightyellow1",
                                  "lightgoldenrod1", "lightgoldenrod2",
                                  "gold", "gold1", "gold2",
                                  "orange1", "sandybrown", "orange2",
...
plot.diff.1 <- ggplot() +
  ...
  geom_point(filter(fishkills, summerkill >= 1),
             ...
             size = 1.5, shape = 21, stroke = 0.25, color = "black", fill = "mediumpurple2") +
  scale_y_continuous(breaks = c(43, 45, 47, 49)) +
  scale_fill_gradientn(limits = c(-0.20, 0.80),
                       colors = c("lightblue3", "lightblue2", "lightblue1",
                                  "white", "lightyellow1", "lightgoldenrod1",
                                  "gold1", "gold2",
                                  "orange1", "sandybrown", "orange2",
                                  "tomato1", "tomato2", "tomato3",
                                  "firebrick3", "firebrick4", "darkred"),
...
```{r, plot, echo = `FALSE`}
# Save figure
pdf("figures/fig_4_diff.pdf", width = 15, height = 3.5) 
...
```{r, plot, echo = `FALSE`}
# Save figure
pdf("figures/fig_S2_ice.pdf", width = 6, height = 4) 

# Boxplot for winterkills ~ ice duration
ggplot() +
  geom_boxplot(winterkills.ice,
               mapping = aes(x = fishkill_cause,
                             y = `Ice duration (days)`,
                             fill = variable),
               outlier.alpha = .3) +
  labs(x = NULL) +
...
```{r, plot, echo = `FALSE`}
# Save figure
pdf("figures/fig_S1_ice.pdf", width = 6, height = 4) 
